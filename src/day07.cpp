...
ErrorCode parseList(Rule &rule, const std::string &list);
ErrorCode nextTwoWords(std::string &output, const std::string &input,
                       const size_t beg);

int numBagsThanCanContain(const std::string &name,
                          const std::map<std::string, Rule> &map,
                          std::set<std::string> &possible);

int numBagsContainedBy(const std::string &name,
                       const std::map<std::string, Rule> &map);
...
ErrorCode execute(const std::string &filename, int &result1, int &result2)
{
    ...
    // With this map, find out the number of elements that have the shiny gold
    // bag as their child.
    std::set<std::string> possible;
                    ...
                    std::map<std::string, Rule> &ruleMap)
{
    ...
    if ((numberPos = line.find_first_of("0123456789", 0)) != std::string::npos)
    {
        ...
        if (parseList(ruleMap[bagName],
                      line.substr(numberPos, line.length() - numberPos)) != Ok)
        {
            Logger::log("Could not parse the rules for at least one bag",
                        ERROR);
            return Error;
        }
    }
